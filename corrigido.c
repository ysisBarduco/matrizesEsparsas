...
// OperaÃ§Ãµes na lista de matrizes
...
void le_dados(Matriz_Esparsa **m, int id){
	...
	do{ 
        ...
        }while(ocupado); // Verifica se a posiÃƒÂ§ÃƒÂ£o digitada nÃƒÂ£o foi preenchida antes de inserir
...
void insere_matriz(Matriz_Esparsa **m, float dado, int lin, int col){
    ...
    if(*m == NULL || //Se a matriz estiver vazia
        ((*m)->lin > lin || // ou a linha atual ÃƒÂ© menor
    ...
    for(i=1; i <= tot_lin; i++){ // Para cada linha
        for(j=1; j <= tot_col; j++){ // Para cada coluna
            if(aux != NULL && aux->lin == i && aux->col == j ){
                printf("%.1f ", aux->dado);
                aux = aux->prox; // AvanÃƒÂ§a quando o nodo for utilizado
            }
            else{
                printf("0.0 ");
            }
...
void matriz_transposta(Lista_Matrizes **l){
    ...
    adicionar_matriz(l, resultado, m->tot_lin, m->tot_col, &novo_id);

    printf("Resultado:\n");
    imprime_matriz(resultado, novo_id, m->tot_lin, m->tot_col);
}
