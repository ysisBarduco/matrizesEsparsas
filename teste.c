...
void insere_matriz(Matriz_Esparsa **m, float dado, int linha, int coluna){
	...
	// Inser��o ordenada por linha e coluna
	if(*m == NULL || // verifica se est� vazio
	...
	while(aux->prox != NULL &&
		...
		aux = aux->prox; // para o pr�ximo ponteiro da lista
	}
...
void le_dados(Matriz_Esparsa **m, int id){
	...
	printf("\n---Leitura da matriz [%d]---\n", id);
	printf("Digite os dados diferentes de 0 e suas posi��es.\nPara finalizar digite 0.\n");
...
void imprime_matriz(Matriz_Esparsa *m, int tot_lin, int tot_col, int id){
	...
	for(i = 1; i <= tot_lin; i++){
		for(j = 1; j <= tot_col; j++){
			...
			// verifica se existe elemento nessa posi��o
			if(aux != NULL && aux->lin == i && aux->col == j){
				printf("%.1f ", aux->dado);
				aux = aux->prox; // avan�a somente quando usa o nodo
			}
			else{
				printf("0.0 ");
			}
...
void imprime_diagonal(lista_matrizes *l){
	...
	matriz_atual = buscar_matriz(l); // salva a posi��o da matriz
	aux = matriz_atual->matriz; // salva a posi��o do primeiro nodo
